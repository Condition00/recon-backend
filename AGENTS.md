...
| points | Complete (DB-only) | Append-only ledger table `point_ledger` with signed deltas and `resulting_balance`; projection table `participant_points` serves leaderboard/rank reads directly from Postgres. Endpoints: `GET /points/me`, `GET /points/leaderboard`, `GET /points/leaderboard/me`, compatibility `GET /leaderboard/me`, `POST /points/award` (admin), `GET /points/transactions` (admin). Redis is no longer part of the points write/read path; event completion and shop redemption both write through the points service into Postgres only. Idempotency key is DB non-null + unique; reason codes validated with namespaced format; participant row locking (`FOR UPDATE`) remains for spend/award concurrency safety. Legacy `points_outbox` infrastructure is retired by migration `f1a2b3c4d5e6`. Test coverage includes in-process router tests, live-server points smoke tests, and optional Postgres-backed concurrency tests (awards, spends, zone registration, and shop stock races) gated by `TEST_POSTGRES_DSN`; one-command test runner: `backend/scripts/test_points_all.ps1`. Current head: `f1a2b3c4d5e6`. |
...
| shop | Complete (initial API) | Full catalogue + redemption store. Tables: `shop_items`, `redemptions`. 8 endpoints: `GET /shop` (list active), `GET /shop/{id}` (detail), `POST /shop/{id}/redeem` (participant redeem with request `idempotency_key`, balance check + stock lock), `GET /shop/me/redemptions` (own history), `GET /shop/redemptions` (admin list, filterable by fulfilled), `PATCH /shop/redemptions/{id}/fulfill` (admin fulfillment), `POST /shop` (admin create), `PATCH /shop/{id}` (admin update/toggle). Concurrent stock safety via `SELECT FOR UPDATE`. Points spending routes through the DB-backed points service with normalized reason codes `shop.redeem.<slug>`, while redemption rows enforce their own unique request idempotency via `redemptions.idempotency_key`. Mounts at `/api/v1/shop/`. Migration: `e6f7a8b9c0d1`. |
